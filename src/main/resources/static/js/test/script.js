...
function quizActions() {
	...
	function sendResult() {
		...
		// open success page
		window.location.href = "/tests/success";
	}
	;
	...
	function checkAns() {
		...
		function showAlert(missing) {
			for (i = 0; i < missing.length; i++) {
				...
				missingAnswer = $('.quizBody.question' + missingAnswer
						+ ' .quizAlert');
...
function buildQuiz(qID, qText, qAnswers, qLength, qCorrectAnswers) {
	...
	for (n = 0; n < qAnswers.length; n++) {
		...
		if ((qAnswers[n].text) && (!qAnswers[n].image)) {
			midCode.push('<div class="col-xs-6"><div class="quizBox" id="'
					+ qAnswers[n].id + '">' + '<div class="ansText">'
					+ qAnswers[n].text + '</div></div></div>');
		}
		// when answers are image-only
		else if ((qAnswers[n].image) && (!qAnswers[n].text)) {
			midCode.push('<div class="col-xs-6"><div class="quizBox" id="'
					+ qAnswers[n].id + '">'
					+ '<div class="ansImage"><img src="' + qAnswers[n].image
					+ '"/></div></div></div>');
		}
		// when answers are text and image
			...
			midCode.push('<div class="col-xs-6"><div class="quizBox" id="'
					+ qAnswers[n].id + '">'
					+ '<div class="ansImage"><img src="' + qAnswers[n].image
					+ '"/>></div><div class="ansText">' + qAnswers[n].text
					+ '</div></div></div>');
		}
	}
	;
	...
	// build HTML in parts
	var quizTop = '<section class="quizBody"><h2 class="quizQuestion"><span class="questionNum">'
			+ qID
			+ '/'
			+ qLength
			+ '</span> '
			+ qText
			+ '</h2><span class="quizAlert">This question requires an answer</span><div class="row">';
